...
def _build_mockup_prompt(payload: GenerateMockupIn) -> str:
    return (
        ...
        "Современный чистый дизайн: аккуратные отступы и читаемая типографика. Макет обязан "
        "быть полностью адаптивным от 320 до 1440 px: добавь meta viewport, используй fluid-"
        "контейнеры (width: 100%, max-width), flex/grid без фиксированной ширины страницы и "
        "минимум один media query для мобильного вида. На ширине 320 px не должно быть "
        "горизонтального скролла: навигация переносится или превращается в компактный вид, "
        "колонки складываются вертикально, текст и кнопки не обрезаются. Проверь desktop, "
        "tablet и mobile композицию. Не добавляй пояснений — только HTML.\n\n"
